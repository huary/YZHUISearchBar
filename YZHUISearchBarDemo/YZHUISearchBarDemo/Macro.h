...
//xx小时xx分xx秒 格式
#define TIME_TEXT_FORMAT_HMS(SEC)                   ((((int)SEC) >= TIME_MIN_SEC) ? ((((int)SEC) >= TIME_HOUR_SEC) ? STRING_FORMAT(@"%d%@%d%@%d%@",((int)SEC)/TIME_HOUR_SEC,TIME_HOUR_TEXT, (((int)SEC)%TIME_HOUR_SEC)/TIME_MIN_SEC,TIME_MIN_TEXT, ((int)SEC)%TIME_MIN_SEC,TIME_SEC_TEXT) : STRING_FORMAT(@"%d%@%d%@",((int)SEC)/TIME_MIN_SEC,TIME_MIN_TEXT, (((int)SEC)%TIME_MIN_SEC),TIME_SEC_TEXT)) : STRING_FORMAT(@"%d%@",(int)SEC,TIME_SEC_TEXT))

//00:00:00格式
#define TIME_TEXT_FORMAT_COLON(SEC)                 ((((int)SEC) >= TIME_MIN_SEC) ? ((((int)SEC) >= TIME_HOUR_SEC) ? STRING_FORMAT(@"%02d:%02d:%02d",(((int)SEC)/TIME_HOUR_SEC), ((((int)SEC)%TIME_HOUR_SEC)/TIME_MIN_SEC), (((int)SEC)%TIME_MIN_SEC)) : STRING_FORMAT(@"00:%02d:%02d",(((int)SEC)/TIME_MIN_SEC), (((int)SEC)%TIME_MIN_SEC))) : STRING_FORMAT(@"00:00:%02d",(int)SEC))

#define TIME_TEXT_FORMAT_COLON_WITH_MSEC(MSEC)      TIME_TEXT_FORMAT_COLON((unsigned int)(MSEC)/TIME_SEC_MSEC)
...
#define MainColor           RGB_WITH_INT_WITH_NO_ALPHA(0x48adf3)//0x35a3dd
...
#else
#define _UI_D_WIDTH         (2048.0)
#define _UI_D_HEIGHT        (1536.0)
#define _UI_D_RATIO         (2.0)
#endif
...
#define UI_WIDTH(W)         REF_LENGTH(_UI_D_WIDTH, W, SCREEN_WIDTH) //((W) * SCREEN_WIDTH /_UI_D_WIDTH)
#define UI_HEIGHT(H)        REF_LENGTH(_UI_D_HEIGHT, H, SCREEN_HEIGHT) //((H) * SCREEN_HEIGHT/_UI_D_HEIGHT)
...
#define UI_FONT(UI_SIZE)    FONT(UI_FONT_SIZE(UI_SIZE))
...
//#define CONST_SINGLE_LINE_WIDTH     (SINGLE_LINE_WIDTH)

#define RIGHT_TEXT                  @"✓"//@"√"//@"✓"
#define WRONG_TEXT                  @"×"
#define SUBJECT_ANSWER_UNKNOW_TEXT  @"未设置答案"
#define SUBJECT_ANSWER_RIGHT        @"正确"
#define SUBJECT_ANSWER_WRONG        @"错误"
...
typedef NS_ENUM(NSInteger, NSEnterVCType)
{
    NSEnterVCTypeHomeAdd       = 0,
    NSEnterVCTypePushNew       = 1,
    NSEnterVCTypeModal         = 2
};
